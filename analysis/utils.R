...
extract_inc_byvax <- function(output, d_E=1/3,includeExposed=TRUE,reduceVac=TRUE) {
  scalefac = ifelse(reduceVac, 1-vp, 1)
  ...
  incid3=data.frame(time=output$time, incid=scalefac*rowSums(d_E*Es[,(2*L+1):(3*L)]), status="Vaccinated")
...
compare_sims <- function(sim1, sim2, name1="1", name2="2",
                             ...
                             stages=NULL,textsize=14, scale_y=TRUE) {
...
ystr = ifelse(scale_y, "per 100K", "")
  p1=  ggplot(data = filter(oo,startDate+15< date), aes(x=date, y=cases, fill=age_band))+theme_light()+
    ...
    ylab(paste("Prevalence"))+scale_x_date( date_breaks = "3 months",date_labels = "%b-%y")
  # scale_x_date( date_labels = "%m-%y")
  
  p2=  ggplot(data = filter(oo,startDate+15< date), aes(x=date, y=hosp*yscaler, fill=age_band))+theme_light()+
    facet_wrap(~scen,nrow = 1) +
    geom_area(position="stack",alpha=0.7)+ guides(fill=FALSE)+
    theme(axis.title.x = element_blank(), 
          text=element_text(size=textsize),
          strip.text.x = element_text(size = textsize+1),
          axis.text.x = element_text(angle = 45, hjust=1))+
    ylab(paste("Hospitalizations", ystr))+scale_x_date( date_breaks = "3 months",date_labels = "%b-%y") #+scale_x_date( date_labels = "%m-%y")
  
  
  p3 = ggplot(data = filter(oo,startDate+15< date), aes(x=date, y=newdeaths*yscaler, fill=age_band))+
    facet_wrap(~scen,nrow = 1) +
    theme_light()+
    geom_area(position="stack",alpha=0.7)+ guides(fill=FALSE)+
    theme(axis.title.x = element_blank(), 
          ...
          strip.text.x = element_text(size = textsize+1),
          axis.text.x = element_text(angle = 45, hjust=1))+
    ylab(paste("Daily deaths",ystr)) +scale_x_date( date_breaks = "3 months",date_labels = "%b-%y")# +scale_x_date( date_labels = "%m-%y")
  p4 = ggplot(data = filter(oo,startDate+15< date), aes(x=date, y=long*yscaler, fill=age_band))+
    facet_wrap(~scen,nrow = 1) +
    theme_light()+
    geom_area(position="stack",alpha=0.7)+guides(fill=FALSE)+
    theme(axis.title.x = element_blank(),
          ...
          strip.text.x = element_text(size = textsize+1),
          axis.text.x = element_text(angle = 45, hjust=1))+
    ylab(paste("Long covid",ystr))+scale_x_date( date_breaks = "3 months",date_labels = "%b-%y")# +scale_x_date( date_labels = "%m-%y")
...
ggplot(data = filter(o1, date>startDate+15), aes(x=date, y=incid*ascFrac, fill=status))+theme_light()+
  ...
  ylab("Incidence") +scale_x_date( date_breaks = "months",date_labels = "%b-%d") + 
  ...
  g2 <- ggplot(res, aes(x=date, y=Proportion, group=Age, col=Age)) +
    ...
    labs(x='',y='', col='') +
            theme(legend.position = "right",
                 axis.title.x = element_blank(),text=element_text(size=8))+
      ggtitle(label) + ylab("Vaccination Percentage")
